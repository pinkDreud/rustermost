...
- **The webview's own storage is limited to non-sensitive UI state.** localStorage holds exactly two keys: `rustermost.url` (the last server URL, prefills the login form) and `rustermost.settings` (display settings: font size, theme, density). The token never touches webview storage.
