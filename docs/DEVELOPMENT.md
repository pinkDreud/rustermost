...
- **The webview's own storage is limited to non-sensitive UI state.** localStorage holds `rustermost.url` (the last server URL, prefills the login form), `rustermost.settings` (display settings: font size, theme, density), `rustermost.muted` (locally silenced conversations), and `rustermost.panes` (resizable-pane sizes: sidebar width + composer floor). The token never touches webview storage.
