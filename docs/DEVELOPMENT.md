...
- **The webview's own storage is limited to non-sensitive UI state.** localStorage holds `rustermost.url` (the last server URL, prefills the login form), `rustermost.settings` (display settings: font size, theme, density), `rustermost.muted` (locally silenced conversations), `rustermost.panes` (resizable-pane sizes: sidebar width + composer floor), `rustermost.spaces` (sidebar spaces: user-named conversation groups), and `rustermost.emojiFont` (the user-loaded custom emoji font: `{ name, data }` with a data: URL). The token never touches webview storage.
- **Custom emoji images persist in IndexedDB.** Database `rustermost`, one store `emoji`, records `{ id, data }` (a data: URL) keyed by emoji id. Mattermost emoji images are immutable per id — a re-upload gets a *new* id, a deleted emoji's id simply leaves the server list — so the cache needs no versioning or etags: at startup the store is folded into memory, entries whose ids the server no longer lists are pruned, and the missing images are prefetched by a small worker pool in the background (this sits in front of the backend's own disk cache, which still costs an invoke round-trip per image per launch). If IndexedDB is unavailable or `open` fails, the app warns once and degrades to in-memory images for the session — every cache helper becomes a no-op.
- **The bundled emoji font is a subset.** `src/fonts/twemoji-mozilla.woff2` starts from the upstream `twemoji-colr-font@15.0.3` package (<https://cdn.jsdelivr.net/npm/twemoji-colr-font@15.0.3/twemoji.woff2>, the Mozilla COLR/CPAL build) and keeps only the glyphs the app actually ships: every emoji value in the picker catalog (`src/emoji-data.js`), the UI chrome glyphs (buttons, menus, chips), and the VS16/ZWJ machinery — ~180 KB instead of ~465 KB. **Anything outside the subset — emoji received in messages, typed on the OS, or added to the catalog later — falls back to the system emoji font**; that is intended, the stacks list `"Rustermost Emoji"` before the generics precisely so the fallback works per-glyph. Regenerate after a catalog or chrome change with fonttools in a throwaway venv (brotli enables woff2): `python3 -m venv /tmp/fontenv && /tmp/fontenv/bin/pip install fonttools brotli`, then `/tmp/fontenv/bin/python packaging/subset-emoji-font.py <full-upstream.woff2> src/fonts/twemoji-mozilla.woff2`. The script re-derives the codepoint list from `src/emoji-data.js`, keeps the chrome constant inline, and refuses to write a font that lost its COLR/CPAL color tables.
