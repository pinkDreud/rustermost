...
- **Sidebar spaces** — group conversations into your own named sections: right-click the sidebar background to create one, then drag conversations in (or use a conversation's right-click menu), and drag onto a default section header to move one back out. They fold like any section and are remembered between launches.
...
- **Emoji** — `:shortcodes:`, an autocomplete popup in the composer (type `:ta…`), a 🙂 browse-and-search picker next to the 📎, and the server's custom emoji rendered inline (cached locally after the first fetch). Emoji render in a flat bundled Twemoji set by default — switchable in ⚙ Settings to the system emoji or a font file of your own.
...
- **Display settings** — a ⚙ panel for font size (small/medium/large), theme (dark/light/system, following the OS live), message density (comfortable/compact), the emoji set, and your Giphy API key; applied instantly and remembered between launches.
- **Resizable layout** — drag the sidebar's right edge or the composer's top edge to size them; the sizes are remembered, and a double-click on the sash resets it.
